...
DEFAULT_CONFIG = ROOT / "data" / "agent_config.json"
...
def collect_active(config: dict):
    """Return (active_models_in_order, active_chat_model). Validates that
    exactly one chat model is active."""
    ...
    active: list[dict] = []
    seen_groups = list(GROUP_ORDER) + [g for g in groups if g not in GROUP_ORDER]
    for g in seen_groups:
        for m in groups.get(g) or []:
            if isinstance(m, dict) and m.get("active") is True:
                active.append(m)
    return active, active_chat[0]
...
def render_model_section(m: dict) -> str:
...
def render_preset(config: dict):
    active, chat = collect_active(config)
    header = (
        "; ============================================================\n"
        ...
        "; boot. Edit data/agent_config.json instead.\n"
        f"; active chat model: {chat.get('name')!r}  (model_id: {chat.get('model_id')!r})\n"
        f"; resident models: {[m.get('model_id') for m in active]}\n"
    ...
    for m in active:
        parts.append("")
        parts.append(render_model_section(m))
    return "\n".join(parts) + "\n", chat, len(active)
